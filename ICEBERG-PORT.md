...
(deliberately blocked rather than routed; the detailed plan is `ICEBERG-SPLICE.md`), the report/pipeline
integration of the lightning-kmp numbers, and some coverage gaps. In order:

### Splice routing (deferred, ~1 week)

Currently blocked loudly, exactly as planned (see the summary in "Explicit exclusions" above).
The detailed design -- the published-nonce session generalizing the closee nonce, the
same-group-key-at-every-index decision, the test plan and the effort estimate -- has its own
document: `ICEBERG-SPLICE.md`.
